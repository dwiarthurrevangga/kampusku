...
export default function PostItem({ post }) {
  ...
  const handleSavePost = async updatedContent => {
    try {
      const { data: p } = await api.put(`/posts/${post.id}`, {
  ...
  const handleAddComment = async text => {
    try {
      ...
      const updated = { ...post, comments: [c, ...comments] };
      setComments(updated.comments);
      setPosts(posts.map(x => x.id === post.id ? updated : x));
    } catch (err) {
  ...
  const handleDeleteComment = async commentId => {
    try {
      ...
      const updated = delRec(comments);
      setComments(updated);
      setPosts(posts.map(x => x.id === post.id ? { ...x, comments: updated } : x));
    } catch (err) {
  ...
  return (
    <>
      ...
      <Modal show={showConfirm} onHide={() => setShowConfirm(false)}>
        ...
        <Modal.Body>Yakin ingin menghapus post ini?</Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
